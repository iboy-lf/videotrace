...
gpu_pair="$("$PY" "$ROOT/scripts/remote/select_gpus.py" \
  ...
  --audit-log "$RUN_DIR/gpu_selection_runtime_profile.json")"
export CUDA_VISIBLE_DEVICES="$gpu_pair"
export VIDEOTRACE_PHYSICAL_GPUS="$gpu_pair"
echo "VideoTrace profile GPUs: physical=$gpu_pair, Qwen=cuda:0, SigLIP=cuda:1"

exec "$PY" scripts/profile_runtime.py "$@"
