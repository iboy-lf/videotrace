...
| 荣耀大模型算法 | LLM/VLM/Agent 统一链路、5/5 错误回归、DPO 准入与 SFT 回退、冷 26.531s/热 15.228s（1.742×）、21/21 缓存命中、当前树 `40/40` 交付校验 | ASR 未部署；不声称从零预训练、通用 entailment 或公开榜单领先；性能是指定单视频任务内 profile，不是公开基准 |
