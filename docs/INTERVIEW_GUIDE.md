...
第一条是直接影响候选选择的 `12 -> 16 -> 8 -> 1` reranker；第二条是 12 条 group-isolated evidence QA 上的 Qwen3.5 BF16 LoRA SFT；第三条是 12 组显式偏好对上的 LoRA DPO；第四条是从偏好对派生的 24 行 answer verifier，导出纯 NumPy 逻辑回归参数。可乐始终只在 frozen test。一步 SFT/DPO 和小 verifier 的指标是链路证据，不是大规模质量宣称。
